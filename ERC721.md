...
Create a new contract file under the contracts directory called `RaboToken.sol`:
```typescript
...
Create a deploy script in the scripts directory called `deploy_RaboToken.ts`:
```typescript
import { ethers } from "hardhat";

async function main() {
  const RaboToken = await ethers.getContractFactory("RaboToken");
  console.log('Deploying RaboToken ERC721 token...');
  const token = await RaboToken.deploy('RaboToken','Rabo');

  await token.deployed();

  console.log("RaboToken deployed to:", token.address);
}
...
Deploy the smart contract using the `deploy_RaboToken.ts` script:
```bash
yarn hardhat run scripts/deploy_RaboToken.ts
```
> Output:
> ```
> RaboToken deployed to: 0x5FbDB2315678afecb367f032d93F642f64180aa3
> ```
...
Again, we write a simple unit test for our RaboToken ERC721 token:
- Check name and symbol
- Mint 2 NFTs

<br />

Create a new file under the test directory called `RaboToken.test.js`:
```typescript
...
describe("RaboToken", function () {

    async function deployRaboTokenFixture() {  
    ...
    describe("Deployment", function () {
      it("Should has the correct name and symbol", async function () {
        const { token, owner } = await loadFixture(deployRaboTokenFixture);
        const total = await token.balanceOf(owner.address);
        expect(total).to.equal(0);
        expect(await token.name()).to.equal('RaboToken');
        expect(await token.symbol()).to.equal('Rabo');
      });
    });

    describe("Mint NFT", function () {
      it("Should mint a token with token ID 1 & 2 to account1", async function () {
        const { token, owner, otherAccount } = await loadFixture(deployRaboTokenFixture);
...
Run unit tests:
```bash
yarn hardhat test test/RaboToken.test.ts
```
> Output:
> ```bash
>   RaboToken
...
Metadata is returned by ERC721 API function tokenURI. Update the `RaboToken.sol` contract with the following:
- Import `base64.sol` and OpenZeppelin Utils Strings.sol
- Implement function tokenURI(tokenId). We create a SVG format image with black background and white tokenId. Name is set as Rabo+tokenId ("Rabo #1" for example). Description is set as "An ERC721 Hardhat tutorial Rabo NFT with on-chain SVG images like look."
```typescript
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.0;
...
contract RaboToken is ERC721 {
    ...
    function tokenURI(uint256 tokenId) override public pure returns (string memory) {
        ...
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "Rabo #', Strings.toString(tokenId), '", "description": "An ERC721 Hardhat tutorial Rabo NFT with on-chain SVG images like look.", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
...
```bash
yarn hardhat run scripts/deploy_RaboToken.ts --network localhost
```
> Output:
> ```bash
> Deploying RaboToken ERC721 token...
> RaboToken deployed to: 0x5FbDB2315678afecb367f032d93F642f64180aa3
> ```
...
const token721 = await ethers.getContractAt("RaboToken", address);
...
> Output:
> ```typescript
> 'data:application/json;base64,'
> ```
